...
class LabelViewMixin:
    def _generate_labels(self, form, field):
        label = form.fields[field].label if field in form.fields else field
        return label


class RenderMessagesOobViewMixin:
    def _render_messages_oob(self):
        return render_to_string(
            "partials/toast_messages.html",
            request=self.request,
        )


class ErrorViewMixin(MessageViewMixin, LabelViewMixin):
    ...
    def _handle_form_errors(self, form):
        for field, errors in form.errors.items():
            for error in errors:
                ...
                self._self_message_error(label, error, extra_tags="vehicle-error")


class VehicleViewMixin(ErrorViewMixin):
    pass
